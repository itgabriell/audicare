...
if (!supabaseUrl || !supabaseKey) {
  console.error('❌ [ChatwootServiceSync] SUPABASE_URL ou SUPABASE_SERVICE_KEY não configurados');
}
...
const ChatwootServiceSync = {
  ...
  /**
   * Sincroniza dados do contato do Chatwoot de volta para o paciente no Audicare
   * @param {Object} contact - Dados do contato do Chatwoot
   * @returns {Promise<Object>} - Resultado da sincronização reversa
   */
  async syncContactBackToPatient(contact) {
    try {
      console.log(`🔄 [SYNC-REVERSE] Sincronizando contato Chatwoot de volta ao Audicare: ${contact.name}`);
      ...
      if (!patientResponse.data.success || !patientResponse.data.patient) {
        console.log(`⚠️ [SYNC-REVERSE] Paciente não encontrado no Audicare para sincronização reversa`);
        return { success: false, reason: 'patient_not_found' };
      }

      const patient = patientResponse.data.patient;

      // Verificar se o contato tem dados adicionais para atualizar no paciente
      const additionalAttributes = contact.additional_attributes || {};

      // Por enquanto, apenas log - podemos expandir para atualizar dados específicos
      console.log(`✅ [SYNC-REVERSE] Contato sincronizado: ${contact.name} ↔ ${patient.name}`);
      ...
      console.error('❌ [SYNC-REVERSE] Erro na sincronização reversa:', error.message);
      return {
        success: false,
        error: error.message
      };
