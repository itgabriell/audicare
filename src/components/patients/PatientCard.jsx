  ...
  return (
    ...
    >
      <div>
        ...
        <div className="space-y-3 mb-5">
          ...
          {(patient.phone || (patient.phones && patient.phones.length > 0)) && (
            <div className="flex items-center justify-between gap-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/50">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0 text-sm">
                  {(() => {
                    ...
                    return <span className="truncate block font-medium text-slate-700 dark:text-slate-300 text-base">{displayPhone}</span>;
                  })()}
                </div>
              </div>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-10 w-10 hover:bg-green-100 hover:text-green-700 rounded-xl transition-colors shrink-0" onClick={handleWhatsAppClick}>
      ...
      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100 dark:border-slate-800/50">
        <Button variant="ghost" className="w-full h-11 text-slate-500 hover:text-slate-900 rounded-xl text-base" onClick={handleEdit}>
          <Edit className="h-4 w-4 mr-2" /> Editar
        </Button>
        <Button variant="ghost" className="w-full h-11 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl text-base" onClick={handleDelete}>
